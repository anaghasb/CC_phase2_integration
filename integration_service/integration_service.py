...
async def link_process_to_partner(link: ProcessPartnerLinkCreate, db: Session = Depends(get_db)):
    """Link a process to an industry partner"""
    ...
    # Create link
    db_link = ProcessPartnerLink(process_id=link.process_id, partner_id=link.partner_id)
    db.add(db_link)
    db.commit()
    db.refresh(db_link)
    return db_link
